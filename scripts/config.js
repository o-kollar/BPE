let data = Alpine.reactive({
    inputText: 'Here is some Text to tokenize',
    encoded:[],
    decoded:"",
    corpus:`Copy paste of the Wikipedia article on Taylor Swift, as of Feb 16, 2024.
    ---
    ...
    numMerges:5,
    maxTokenLength:5,
    regex: /'(?:[sdmt]|ll|ve|re)|\b(?:\d{1,4}[-/.]\d{1,2}[-/.]\d{2,4}|\d{1,2}[-/.]\d{1,2}[-/.]\d{2,4}|\d{2,4})\b|[\p{L}\p{M}]+|[\p{N}\p{Nd}]+|[^\s\p{L}\p{N}]+|\s+/gu,
    recontsError:0
})